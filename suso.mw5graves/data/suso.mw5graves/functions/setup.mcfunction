scoreboard objectives add suso.mw5graves.color dummy
scoreboard objectives add suso.mw5graves.var dummy
scoreboard objectives add suso.mw5graves.id dummy

scoreboard objectives add suso.mw5graves.death deathCount

gamerule keepInventory true

forceload add 29999984 27115040
setblock 29999984 1 27115040 minecraft:yellow_shulker_box[facing=down]